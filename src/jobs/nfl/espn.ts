...
export const createEspnNflJob = (espnMatchId: string): OracleJob => {
  const espnCompletedMatch = `$.competitions[?(@.status.type.completed && @.id == '${espnMatchId}')]`;
  return OracleJob.create({
    tasks: [
      OracleJob.Task.create({
        // The first task hits the ESPN API using the proper match ID (For example 401326441).
        httpTask: OracleJob.HttpTask.create({
          url: `https://site.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard/${espnMatchId}`,
        }),
      }),
